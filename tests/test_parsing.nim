...
suite "Loading":
  ...
  suite "from buffer":
    var 
      file = exePath.open
      pseudoBuffer = newSeq[byte](file.getFileSize)

    defer: file.close
    check: file.readBytes(pseudoBuffer, 0, file.getFileSize) == pseudoBuffer.len

    var p = PEImage.newFromPtr(cast[pointer](addr(pseudoBuffer[0])))
    p.audacityVerify
